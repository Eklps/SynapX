...
public abstract class AbstractMessageHandler {
    ...
    public <T> T chat(ChatContext chatContext, MessageTransport<T> transport) {
        ...
        try {
            ...
            logger.error("对话处理异常: sessionId={}, error={}", chatContext.getSessionId(), e.getMessage(), e);
            // 通过已建立的连接发送SSE格式的错误消息，避免前端一直显示"思考中"
            try {
                transport.sendMessage(connection,
                        AgentChatResponse.buildEndMessage("对话处理失败: " + e.getMessage(), MessageType.TEXT));
