...
public class ContainerAppService {
    ...
    private String extractIpAddress(DockerService.ContainerInfo containerInfo, String networkMode) {
        // 检测当前后端是否运行在Docker容器内
        // 如果是，需要使用 host.docker.internal 而不是 localhost
        String hostAddress = isRunningInDocker() ? "host.docker.internal" : "localhost";

        // host网络模式
        if ("host".equals(networkMode)) {
            logger.info("容器使用host网络模式，IP地址设为: {}", hostAddress);
            return hostAddress;
        }

        // bridge网络模式，通过端口映射访问
        if ("bridge".equals(networkMode)) {
            logger.info("容器使用bridge网络模式，IP地址设为: {}（通过端口映射访问）", hostAddress);
            return hostAddress;
        }
